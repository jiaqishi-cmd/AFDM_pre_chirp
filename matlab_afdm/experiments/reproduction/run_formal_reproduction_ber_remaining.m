function run_formal_reproduction_ber_remaining()
%RUN_FORMAL_REPRODUCTION_BER_REMAINING Continue formal reproduction from BER figures.
    ...
    logPath = fullfile(outputDir, ['formal_repro_ber_remaining_' timestamp '.log']);
    diary(logPath);
    cleanup = onCleanup(@() diary('off'));

    fprintf('Formal BER remaining steps started at %s\n', datestr(now));
    fprintf('Root: %s\n', rootDir);
    fprintf('Log: %s\n\n', logPath);
    ...
    fprintf('\nFormal BER remaining steps finished at %s\n', datestr(now));
end
