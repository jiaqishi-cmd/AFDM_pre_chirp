...
%SCAN_THETA_DMIN_CASEA �?Case A 扫描两径增益相位并计算有限星�?dmin�?%   主指标是 dmin2，不�?total H_eff rank 作为结论；rank/cond 仅辅助排查�?
    if nargin < 3
        options = struct();
    end
    ...
    paths.gps.H1 = afdm.analysis.build_path_matrix(N, c1, c2Gps, l1, alpha1);
    paths.gps.H2 = afdm.analysis.build_path_matrix(N, c1, c2Gps, l2, alpha2);
    if hasProposed
        paths.prop.H1 = afdm.analysis.build_path_matrix(N, c1, c2Prop, l1, alpha1);
        paths.prop.H2 = afdm.analysis.build_path_matrix(N, c1, c2Prop, l2, alpha2);
    end
