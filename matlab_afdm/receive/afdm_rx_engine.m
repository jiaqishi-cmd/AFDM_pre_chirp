function [x_dec, err_bits, total_bits] = afdm_rx_engine(r_signal, config, x_ref, rx_state)
%AFDM_RX_ENGINE Run the AFDM receiver chain and compute bit errors.
%   [x_dec, err_bits, total_bits] = afdm_rx_engine(r_signal, config, x_ref, rx_state)

    if nargin >= 4 && isstruct(rx_state) && isfield(rx_state, 'c2')
        c2 = rx_state.c2;
    else
    ...
    if isfield(config.channel, 'multipath') && ~config.channel.multipath
        H_eff = eye(config.waveform.NumSubcarriers);
    else
